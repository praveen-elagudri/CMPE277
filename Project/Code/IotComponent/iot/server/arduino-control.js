module.exports = function(app, io) {
	var bodyParser = require('body-parser'),
		...
		five = require('johnny-five'),//Jonnyfive
	...
	board.on('ready', function() {
		...
		tempSensor.on('read', function(err, value){
			...
			console.log(consolePrefix + 'Temperature is ' + temp);
			temperature = temp;
		});
...
var sensor_data = function() {
	var timestamp= Date();
request.post(
    'https://packers-backend.herokuapp.com/sensor_data', //post rest api
    {json: {
    		timestamp,
    		temperature,
    		brightness,
...
var led_response = function() {
	...
	var emptyRoom_function = function() {
		...
		if (brightness>30 && ( (m1>(m2-3)) && (m1<(m2+3)) ) ){ // condition for empty room
			i++;
		...
		m2=m1;
		console.log(consolePrefix +"----------------------------------------------------------------Counter value is i:"+i);
